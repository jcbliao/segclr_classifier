...
function draw(){const p=cells.filter(r=>r.model===val(model)),types=uniq(p.map(r=>r.cell_type)).sort(),traces=[],layout={height:550+280*Math.ceil(types.length/5),title:{text:`${val(model)} · per-cell F1 CDF by granular cell type · fold_0`,font:{size:16}},margin:{l:70,r:20,t:75,b:65},shapes:[],annotations:[]};
