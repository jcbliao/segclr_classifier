...
#plot{background:white;margin:0}
...
<div class="controls"></div><div class="plotwrap"><div id="plot"></div></div><div class="note">Use the toolbar to pan, zoom, reset, or download a PNG. Narrow embeds scroll horizontally instead of compressing the notebook-sized figure.</div>
...
function draw(){let traces=[];for(const n of [10,20,40]){const p=rows.filter(r=>r.n_embeddings===n).sort((a,b)=>b.window_macro_f1-a.window_macro_f1);
 ...
 const layout={width:1900,height:700,grid:{rows:1,columns:3,pattern:'independent'},margin:{l:210,r:25,t:65,b:65},annotations:[10,20,40].map((n,i)=>({text:`${n} embeddings`,xref:`x${i?i+1:''} domain`,yref:'paper',x:.5,y:1.06,showarrow:false,font:{size:16}}))};for(let i=1;i<=3;i++){layout['xaxis'+(i===1?'':i)]={title:'window macro F1',gridcolor:'#d9d9d9'};layout['yaxis'+(i===1?'':i)]={automargin:true,autorange:'reversed'}}Plotly.react(gd,traces,layout,config)}draw();""")
...
function draw(){const c=val(cs),scope=val(level),m=val(metric);let col;if(c===null){col=scope+'_'+({f1:'macro_f1',recall:'balanced_accuracy',precision:'macro_precision',accuracy:'accuracy'}[m])}else col=scope+'_'+(m==='accuracy'?'recall':m)+'_'+c;
 ...
 const ncols=3,nrows=Math.ceil(architectures.length/ncols),layout={width:1500,height:500*nrows,grid:{rows:nrows,columns:ncols,pattern:'independent'},margin:{l:75,r:25,t:75,b:110},legend:{orientation:'h',y:-.08},annotations:architectures.map((a,i)=>({text:a,xref:`x${i?i+1:''} domain`,yref:`y${i?i+1:''} domain`,x:.5,y:1.13,showarrow:false,font:{size:15}}))};architectures.forEach((_,i)=>{const k=i?'axis'+(i+1):'axis';layout['x'+k]={title:'Number of embeddings',tickvals:[10,20,40],gridcolor:'#d9d9d9'};layout['y'+k]={title:(c||'macro')+' '+m,gridcolor:'#d9d9d9'}});Plotly.react(gd,traces,layout,config)}draw();""")
    ...
    page("architecture-confusion-matrices", "Confusion matrices across node count",
         ...
         "const mplBlues=" + json.dumps([
             [i / 255, colors.to_hex(colormaps["Blues"](i / 255))]
             for i in range(256)
         ], separators=(",", ":")) + ";" + r"""
...
arch.addEventListener('change',()=>{sync();draw()});model.addEventListener('change',draw);function draw(){const p=rows.filter(r=>r.architecture===val(arch)&&r.model===val(model)).sort((a,b)=>a.n-b.n),traces=[],annotations=[];let rawMax=0;
 p.forEach((r,i)=>{const m=r[val(level)],z=m.confusion_matrix.map(x=>x.slice()),normalized=val(norm)==='normalized',axis=i?String(i+1):'';if(normalized)z.forEach(row=>{const s=row.reduce((a,b)=>a+b,0);row.forEach((x,j)=>row[j]=s?x/s:0)});else z.flat().forEach(x=>rawMax=Math.max(rawMax,x));traces.push({type:'heatmap',z,x:r.classes,y:r.classes,xaxis:'x'+axis,yaxis:'y'+axis,coloraxis:'coloraxis',showscale:false,hovertemplate:'true %{y}<br>predicted %{x}<br>%{z:'+(normalized?'.2f':',d')+'}<extra></extra>'});
  annotations.push({text:`n=${r.n} · macro F1 ${m.macro_f1.toFixed(3)} · balanced acc ${m.balanced_accuracy.toFixed(3)}`,xref:`x${axis} domain`,yref:'paper',x:.5,y:1.04,showarrow:false,font:{size:12}});z.forEach((row,ri)=>row.forEach((v,ci)=>annotations.push({text:normalized?v.toFixed(2):Math.round(v).toLocaleString(),x:r.classes[ci],y:r.classes[ri],xref:'x'+axis,yref:'y'+axis,showarrow:false,font:{size:10,color:v>(normalized?.5:rawMax/2)?'white':'black'}})))});
 const layout={width:2250,height:750,title:{text:`${val(model)} · ${val(level)} level${val(norm)==='raw'?' · raw counts':''}`,font:{size:16}},grid:{rows:1,columns:Math.max(1,p.length),pattern:'independent'},coloraxis:{colorscale:mplBlues,cmin:0,cmax:val(norm)==='normalized'?1:undefined,colorbar:{thickness:14}},margin:{l:115,r:55,t:90,b:145},annotations};p.forEach((r,i)=>{const k=i?'axis'+(i+1):'axis';layout['x'+k]={title:'Predicted class',tickangle:-45,automargin:true,constrain:'domain'};layout['y'+k]={title:i===0?'True class':'',autorange:'reversed',automargin:true,scaleanchor:'x'+(i?i+1:''),constrain:'domain',showticklabels:i===0}});Plotly.react(gd,traces,layout,config)}sync();draw();""")
...
function draw(){const c=val(cls);let p=rows.bins.filter(r=>(c===null?r.class_name===null:r.class_name===c));"""+(r"p=p.filter(r=>r.model===val(model));" if family else "")+r"""
 ...
 const ncols=3,nrows=Math.ceil(panels.length/ncols),layout={width:"""+("1650" if family else "1500")+r""",height:"""+("420" if family else "400*nrows")+r""",grid:{rows:nrows,columns:ncols,pattern:'independent'},margin:{l:75,r:20,t:70,b:90},legend:{orientation:'h',y:-.08},annotations:panels.map((x,i)=>({text:"""+("'n='+x" if family else "x")+r""",xref:`x${i?i+1:''} domain`,yref:`y${i?i+1:''} domain`,x:.5,y:1.13,showarrow:false,font:{size:14}}))};panels.forEach((_,i)=>{const k=i?'axis'+(i+1):'axis';layout['x'+k]={title:p.length?p[0].feature.replaceAll('_',' '):'feature',gridcolor:'#d9d9d9'};layout['y'+k]={title:(c||'macro')+' F1',gridcolor:'#d9d9d9',matches:'y'}});Plotly.react(gd,traces,layout,config)}"""+("sync();" if family else "")+"draw();"
...
function draw(){const p=rows.filter(r=>r.model===val(model)&&r.granular===true);const names=uniq(p.map(r=>r.class_name));const traces=[10,20,40].map((n,i)=>({type:'bar',name:`n=${n}`,x:names,y:names.map(x=>{const q=p.find(r=>r.n_embeddings===n&&r.class_name===x);return q?q.f1:null}),hovertemplate:'%{x}<br>F1 %{y:.3f}<extra></extra>'}));Plotly.react(gd,traces,{width:Math.max(1200,55*names.length),height:550,barmode:'group',yaxis:{range:[0,1],title:'window F1'},xaxis:{tickangle:-55},margin:{l:65,r:20,t:30,b:170}},config)}sync();draw();""")

    data=json.dumps({
        "cells": json.loads(records(cells,["architecture","model","n_embeddings","root_id","cell_type","f1"])),
        "overall": json.loads(records(overall,["model","n_embeddings","class_name","f1"])),
    }, separators=(",", ":"))
...
const cells=rows.cells,overall=rows.overall;const arch=control('arch','Architecture',uniq(cells.map(r=>r.architecture)));const model=control('model','Ablation',[]);function sync(){model.innerHTML='';uniq(cells.filter(r=>r.architecture===val(arch)).map(r=>r.model)).forEach((v,i)=>{const o=new Option(v,i);o._value=v;model.add(o)})}arch.addEventListener('change',()=>{sync();draw()});model.addEventListener('change',draw);
function draw(){const p=cells.filter(r=>r.model===val(model)),types=uniq(p.map(r=>r.cell_type)).sort(),traces=[],layout={width:2000,height:550+280*Math.ceil(types.length/5),title:{text:`${val(model)} · per-cell F1 CDF by granular cell type · fold_0`,font:{size:16}},margin:{l:70,r:20,t:75,b:65},shapes:[],annotations:[]};
 function curve(part,n,axis,show){const x=part.filter(r=>r.n_embeddings===n).map(r=>r.f1).sort((a,b)=>a-b);if(x.length){let label=`n=${n} · ${x.length} cells`;const macro=overall.find(r=>r.model===val(model)&&r.n_embeddings===n&&r.class_name===null);if(macro)label+=` · window macro F1 ${macro.f1.toFixed(3)}`;traces.push({x,y:x.map((_,i)=>(i+1)/x.length),xaxis:'x'+axis,yaxis:'y'+axis,mode:'lines',line:{shape:'hv',color:palette[[10,20,40].indexOf(n)]},name:label,legendgroup:`n=${n}`,showlegend:show,hovertemplate:'F1 %{x:.3f}<br>fraction ≤ %{y:.3f}<extra></extra>'})}}
 [10,20,40].forEach(n=>curve(p,n,'',true));layout.xaxis={domain:[0,1],anchor:'y',range:[0,1],title:'per-cell F1'};layout.yaxis={domain:[.58,1],anchor:'x',range:[0,1],title:'fraction of held-out cells'};layout.annotations.push({text:'All held-out cells',xref:'paper',yref:'paper',x:.5,y:1.01,showarrow:false,font:{size:15}});layout.shapes.push({type:'line',xref:'x',yref:'y',x0:.5,x1:.5,y0:0,y1:1,line:{dash:'dash',color:'#555'}});
 const cols=5,nrows=Math.ceil(types.length/cols),gap=.025,w=(1-gap*(cols-1))/cols,rowH=.48/nrows;types.forEach((type,i)=>{const id=i+2,a=String(id),col=i%cols,row=Math.floor(i/cols),x0=col*(w+gap),y1=.50-row*rowH,y0=y1-rowH*.78,typed=p.filter(r=>r.cell_type===type),nCells=uniq(typed.map(r=>r.root_id)).length;layout['xaxis'+a]={domain:[x0,x0+w],anchor:'y'+a,range:[0,1],title:'per-cell F1'};layout['yaxis'+a]={domain:[y0,y1],anchor:'x'+a,range:[0,1]};[10,20,40].forEach(n=>curve(typed,n,a,false));layout.annotations.push({text:`${type} · ${nCells} cells`,xref:'paper',yref:'paper',x:x0+w/2,y:y1+.02,showarrow:false,font:{size:11}});layout.shapes.push({type:'line',xref:'x'+a,yref:'y'+a,x0:.5,x1:.5,y0:0,y1:1,line:{dash:'dash',color:'#555'}})});Plotly.react(gd,traces,layout,config)}sync();draw();""")
